...
from ksuid import KSUID, generate, generate_token, from_string
...
class PrefixedKSUID:
    ...
    def create(cls, prefix: str) -> str:
        ...
        # No underscores in prefix since _ is the delimiter
        if not re.match(r"^[a-zA-Z][a-zA-Z0-9]*$", prefix):
            raise ValueError(
                "Prefix must start with a letter and "
...
def demo_validation():
    ...
    # Valid cases
    is_user = PrefixedKSUID.validate(user_id, "user")
    print(f"Is '{user_id}' a valid user ID? {is_user}")
    is_pay = PrefixedKSUID.validate(payment_id, "pi")
    print(f"Is '{payment_id}' a valid payment ID? {is_pay}")
    ...
    print(
        f"Is 'user_invalid_ksuid' valid? {PrefixedKSUID.validate('user_invalid_ksuid')}"
    )
    print()
...
def demo_database_patterns():
    ...
    for i in range(5):
        ...
        records.append(
            {"id": record_id, "type": record_type, "data": f"Sample {record_type} {i}"}
        )
